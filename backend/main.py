...
# # Removed: from vertexai.generative_models import GenerativeModel, Tool # We will use google.genai.Client directly
# from google.cloud import storage, firestore
# import requests # Import requests for HTTP calls to the new service

# # Load environment variables from .env file
# # The path is now relative to the root directory where the app is run
...
# # Initialize clients
# # Set environment variables for the SDK to use Vertex AI as per the documentation
...
# REQUIREMENT_EXTRACTOR_SERVICE_URL = os.getenv("REQUIREMENT_EXTRACTOR_SERVICE_URL") # New environment variable
...
# # New Endpoints for Document Processing and Test Case Generation
# @app.get("/documents")
# async def get_user_documents(user_id: str):
#     try:
#         docs_query = db.collection("document_jobs").where(field_path="user_id", op_string="==", value=user_id).order_by("created_at", direction=firestore.Query.DESCENDING)
#         results = docs_query.stream()
        
#         documents = []
#         for doc in results:
#             doc_data = doc.to_dict()
#             doc_data["id"] = doc.id
#             doc_data["name"] = doc_data.get("original_filename", "Untitled Document") # Add this line to map original_filename to name
#             # Recalculate overall status for the list view
...
#         docling_status = job_data.get("docling_status")
#         indexing_status = job_data.get("indexing_status")
...
#             # The file name in the corpus is derived from the GCS URI
...
#         # Check for completion of both parallel processes
#         if job_data.get("docling_status") != "Completed" or job_data.get("indexing_status") != "Completed":
#             raise HTTPException(status_code=400, detail=f"Document processing is not complete. Docling: {job_data.get('docling_status')}, Indexing: {job_data.get('indexing_status')}")
...
#             raise HTTPException(status_code=500, detail="REQUIREMENT_EXTRACTOR_SERVICE_URL is not configured.")

#         # 1. Call Requirement Extraction Service
#         extraction_payload = {
#             "document_id": document_id,
#             "user_id": user_id
#         }
#         extraction_response = requests.post(REQUIREMENT_EXTRACTOR_SERVICE_URL, json=extraction_payload)
#         extraction_response.raise_for_status()
#         extracted_data = extraction_response.json()
#         extracted_requirements = extracted_data.get("requirements", [])

#         if not extracted_requirements:
#             raise HTTPException(status_code=400, detail="No requirements extracted from the document.")

#         # For now, we'll generate test cases for all extracted requirements in one go.
#         # In future iterations, this would be broken down into scenario generation and then detailed test case generation per scenario.
        
#         # Combine all requirement descriptions for the main prompt
#         requirements_text = "\n".join([f"- {req['id']}: {req['description']}" for req in extracted_requirements])

#         if not RAG_RETRIEVAL_SERVICE_URL:
#             raise HTTPException(status_code=500, detail="RAG_RETRIEVAL_SERVICE_URL is not configured.")

#         # Prepare query for the custom retrieval service using all requirements
#         retrieval_query_text = f"Generate test cases for the following requirements: {requirements_text} focusing on {', '.join(test_types)} with {', '.join(priorities)} priority and compliance with {', '.join(standards)}."
...
#         # Call the custom retrieval service
#         retrieval_response = requests.post(RAG_RETRIEVAL_SERVICE_URL, json=retrieval_payload)
#         retrieval_response.raise_for_status() # Raise an exception for HTTP errors
#         retrieved_data = retrieval_response.json()
#         combined_contexts = retrieved_data.get("contexts", [])

#         # Construct the prompt with extracted requirements and retrieved contexts
#         prompt_parts = [
...
#             prompt_parts.append("Each test case MUST be fully compliant with all necessary healthcare regulations and standards.")
#         else:
#             prompt_parts.append("Each test case MUST be fully compliant with all necessary healthcare regulations and standards (like FDA, ISO 13485, etc.).")

#         prompt_parts.append("The final output should be a list of traceable, compliant test cases.")
#         prompt = "\n".join(prompt_parts)
        
#         # Generate content using the google-genai client directly
...
#         # Store the generated test cases in Firestore
#         doc_ref.update({"test_cases": response.text, "generated_at": firestore.SERVER_TIMESTAMP})
